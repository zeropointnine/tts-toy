class ConstantsLong:
    ...
    *   Any other symbols used purely for visual separation or emphasis that are not typically spoken aloud (e.g., using multiple hyphens `---` as a divider).
    ...
    TEST_TEXT_1 = "This is the first sentence. This is a second sentence, which is a bit longer and might need splitting based on word count, but this part right here is also quite lengthy and might itself require a hard split even after the comma separation. Here is a third sentence; it uses a semicolon. A fourth: with a colon. This fifth sentence is deliberately made very long to ensure that the splitting mechanism based on word count and phrase separators like commas, semicolons, or colons is triggered effectively, hopefully creating multiple chunks from this single sentence alone. Mr. Smith went to Washington D.C. for a meeting. What about questions? This should work!"

    TEST_TEXT_2 = "This single sentence is extremely long and contains no standard punctuation like commas or semicolons for easy splitting, therefore it must be broken down using the hard word count limit to ensure that no single chunk exceeds the maximum allowed words for the target duration."

    TEST_TEXT_3 = "Two years ago, a friend of mine asked me to say some MC rhymes, so I said this rhyme I'm about to say, the rhyme was deffer when it went this way."

    TEST_TEXT_4 = """Oh, that's a good question <giggle>
